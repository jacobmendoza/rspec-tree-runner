...
module.exports = {
  ...
  activate() {
    ...
    this.subscriptions.add(
      ...
      atom.commands.add('atom-workspace', 'rspec-tree-runner:run-tests', () => this.runTests(this.currentState.runTests.bind(this.currentState))),
      atom.commands.add('atom-workspace', 'rspec-tree-runner:run-single-test', () => this.runTests(this.currentState.runSingleTest.bind(this.currentState)))
    );
  ...
  handleEditorEvents(editor) {
    ...
    currentBuffer.onDidSave(() => {
      if (this.currentState.file.isValidSpecFile()) {
        this.updateStateWith(editor);
      }
    });
    this.updateStateWith(editor);
  },
  updateStateWith(editor) {
    this.view.show();
    ...
    this.view.setUiForRubyFile(this.currentState.file.name);
